module.exports = async ({
  ...
  if (context.eventName === "workflow_dispatch" && custom_paths) {
    ...
    if (pull_request_number) {
      ...
      try {
        ...
        if (error.status === 403) {
          ...
          console.log(
            "Received a 403 error. Permission denied to access the resource."
          );
        } else {
  ...
  if (directories.size > 0) {
    ...
    core.exportVariable(
      ...
      Array.from(directories).join("\n")
    );

    const workspacePath = process.env.GITHUB_WORKSPACE;
    const fullPaths = Array.from(directories).map(
