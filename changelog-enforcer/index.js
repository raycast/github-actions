module.exports = async ({ github, context, core, custom_paths }) => {
    try {
        ...
        if (labelNames.includes(skipChangeLogLabel)) {
            console.log(`Skipping changelog checks: found "${skipChangeLogLabel}" label on PR`)
            return
        }
        ...
        extensions.forEach(extension => {
            ...
            const changelogExists = files.some(file => file.filename === changelogPath);
            if (!changelogExists) {
        ...
        if (missingChangelogs.length > 0) {
            core.setFailed(`Missing ${changelogFileName} update for: ${missingChangelogs.join(", ")} `);
        }
    } catch (error) {
